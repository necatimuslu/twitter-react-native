...
import { getTweetByUserId } from "../../services/tweetService";
import HomeCard from "../components/HomeCard";

const FriendProfile = ({ route }) => {
  const [users, setUsers] = useState([]);
  const navigation = useNavigation();
  const user = route.params.t;
  useEffect(() => {
    getTweetByUserId({ userId: route.params.userId })
      .then((res) => {
        setUsers(res.data);
      })
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <ScrollView>
        {users && users.map((user, i) => <HomeCard key={i} t={user} />)}
      </ScrollView>
      {/*   <View>
